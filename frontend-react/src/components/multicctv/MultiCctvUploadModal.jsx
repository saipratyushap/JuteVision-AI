...
import { Video, Grid2x2, Settings2, Upload, X } from 'lucide-react'
import Modal from '../ui/Modal'
import { cn } from '../../utils/helpers'

const LAYOUTS = [
  { id: '1', label: '1', sub: 'Single', rows: 1, cols: 1, slots: 1, icon: Video },
...
export default function MultiCctvUploadModal({ open, onClose, onStart }) {
  const [layoutId, setLayoutId] = useState('2x2')
  const [files, setFiles] = useState([])
  const [error, setError] = useState('')
  const fileRef = useRef(null)

  const layout = useMemo(() => LAYOUTS.find(l => l.id === layoutId) ?? LAYOUTS[3], [layoutId])
  ...
  const handleFiles = (nextList) => {
    ...
    if (next.length > slots) {
      setError(`You selected ${next.length} files. Using the first ${slots} for ${layout.label}.`)
  ...
  const handleStart = () => {
    setError('')
    if (files.length < 1) {
      setError('Please select at least 1 video file to start processing.')
      return
    }
    ...
    <Modal open={open} onClose={onClose} size="xl" title={null}>
      <div className="p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-xl font-black text-slate-900 dark:text-white">Upload Videos to Multi-Camera Grid</h3>
            <p className="text-sm text-slate-500 dark:text-gray-500 mt-1">Select a layout, then pick individual video files for each camera.</p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-black/10 dark:border-white/8 bg-black/5 dark:bg-white/3 p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm font-bold text-slate-900 dark:text-white truncate">Selected videos</p>
